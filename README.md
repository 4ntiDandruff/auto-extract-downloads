...
[![OS - Linux](https://img.shields.io/badge/OS-Linux%20(KDE%20%7C%20GNOME%20%7C%20XFCE)-blue?style=for-the-badge&logo=l…
[![Desktop - KDE Plasma 6](https://img.shields.io/badge/Desktop-KDE%20Plasma%206%20%7C%20Wayland-1d99f3?style=for-the-b…
[![Core - Bash POSIX](https://img.shields.io/badge/Engine-POSIX%20Bash%20%2B%20inotify-4EAA25?style=for-the-badge&logo=…
[![Process - Systemd](https://img.shields.io/badge/Daemon-Systemd%20User%20Unit-CC2200?style=for-the-badge&logo=systemd…
[![Version - v1.2.0](https://img.shields.io/badge/Release-v1.2.0%20Stable-success?style=for-the-badge)](https://github.…
<p align="center">
...
Pada lingkungan Linux Desktop (KDE Plasma, GNOME, XFCE), jendela dialog pemilihan file (*File Picker / `KFileDialog`*) …
**Auto-Extract Downloads** memecahkan masalah tersebut pada tingkat fondasi sistem. Menggunakan arsitektur *event-drive…
---
...
|---|---|---|
| **Kernel Event Layer** | `inotify-tools` (`inotifywait`) | Menerima sinyal hardware interrupt (`IN_CLOSE_WRITE`, `IN_…
| **Worker Engine** | POSIX Bash Script | Logika parsing ekstensi, kedalaman direktori, evaluasi 9 sekring, & triggerin…
| **Archive Backend** | `unar` (The Unarchiver) & `7z` | Ekstraksi multi-format universal & deteksi otomatis arsip tere…
| **Process Manager** | Systemd User Unit (`systemd --user`) | Manajemen daemons, auto-start sesi login, & isolasi reso…
| **Desktop Alerting** | `libnotify` (`notify-send`) | Notifikasi visual asinkron non-blocking pada panel tray desktop …
| **Dynamic CLI Switch** | `/usr/local/bin/auto-extract` | Antarmuka pengguna untuk konfigurasi dinamis (`limit`, `dept…
---
...
│                                                                                  │
│   ├── [Sekring 1: Self-Compress Loop Guard] ► (Mencegah loop saat kompres folder sendiri) │
...
- **Zero-Compilation (No-Build Step)**: Mengeliminasi runtime overhead (Node.js/Python/Rust compiler) di tingkat backgr…
- **Konsumsi Daya & CPU 0.0% (Zero Idle Polling)**: Menggunakan mekanisme *Linux Inotify Subsystem*, proses tidur (*sle…
- **Footprint Memori Ultra Ringan**: Berjalan pada `user.slice` dengan alokasi RAM hanya **~1.0 MB – 1.9 MB** (jauh leb…
...
- 🛡️ **Implementasi 9 Lapis Sekring Proteksi (Safety Fuses)**:
  1. `Self-Compress Loop Guard`: Mencegah loop ekstraksi arus balik saat pengguna mengompres folder sendiri.
  ...
  9. `Image & Container Protection`: Proteksi mutlak file `.iso`, `.bin`, `.rom`, `.img`, `.vdi`, `.qcow2`, `.vmdk`, `.…
- 🎛️ **Dynamic CLI Configuration (Fase 2)**: Pengaturan sekring langsung via command terminal tanpa bongkar file script…
---
...
|---|---|---|---|
| **Waktu Ekstraksi per File** | 10 – 20 detik (Buka GUI, cari direktori, ekstrak) | **< 0.5 detik (Instan Latar Belaka…
| **Friksi Dialog Import** | Pindah window → ekstrak manual → kembali ke dialog | File langsung matang siap dipilih di …
| **Konsumsi RAM Desktop** | GUI Archive Manager (~80 MB – 150 MB RAM) | Background Worker (**~1.5 MB RAM**) | Penghema…
| **Valuasi Waktu Teknisi** | 15x unduhan/hari = ~2.5 jam terbuang/bulan | Eksekusi instan tanpa campur tangan user | *…
---
## 8. Roadmap & Rencana Pengembangan
| Tahapan Rilis | Target Fitur & Skalabilitas | Nilai Tambah bagi Pengguna |
|---|---|---|
| **Fase 1 (v1.0.0)** | 6 Sekring Proteksi Dasar + Systemd Unit + CLI Switch | Stabilitas pondasi & integrasi desktop d…
| **Fase 2 (v1.2.0 - Aktif)** | Rekursif Subfolder + 9 Sekring Proteksi + Dynamic CLI Config | Fleksibilitas penuh & pr…
| **Fase 3 (Berikutnya)** | KDE Plasma System Tray Toggle / KRunner Runner Plugin | Pengendalian visual interaktif via …
