...
│                                                                                  │
│   ├── [Sekring 1: Self-Compress Loop Guard] ► (Skip jika folder sudah ber-isi)   │
│   ├── [Sekring 2: Dynamic Limit Ukuran] ──► (Skip jika ukuran > MAX_SIZE_MB)     │
│   ├── [Sekring 3: Multi-Part Guard] ──────► (Hanya trigger part1, abaikan part2) │
│   ├── [Sekring 4: Disk Undervoltage] ─────► (Batal jika free disk < MIN_DISK_GB) │
│   ├── [Sekring 5: Password Guard] ────────► (Deteksi enkripsi -> minta manual)   │
│   ├── [Sekring 6: Auto-Rollback] ─────────► (Bersihkan folder jika korup/gagal)  │
│   ├── [Sekring 7: Max Depth Guard] ───────► (Batasi kedalaman traversal folder)  │
│   ├── [Sekring 8: Dev Exclude Folders] ───► (Abaikan .git/, node_modules/, dll)  │
│   └── [Sekring 9: Image & Container Guard]► (Proteksi ISO, BIN, ROM, VDI, TAR)   │
└────────────────────────────────────────┬─────────────────────────────────────────┘
